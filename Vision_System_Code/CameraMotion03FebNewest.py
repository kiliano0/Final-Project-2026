    ...
    def run(self):
        while self.running:
            ...
            try:
                ...
                # Keep reading lines until READY (or ERROR) so the socket buffer doesn't clog
                final_reply = None
                while True:
                    ...
                    # Stop conditions for this command
                    if reply_u == "READY":
                        final_reply = reply_u
                        break

                    # Otherwise: ignore extra status lines and keep reading
...
def calibration_loop(picam2):
    """
    Click 4 corners.
    Uses AUTO_WORLD_PTS automatically (no console input).
    Returns homography.
    """
    image_points = []
    homography = None
...
def detect_rectangle(frame_bgr, homography):
    ...
    # HSV mask (looser than black-only)
    mask = cv2.inRange(hsv, (int(H_MIN), int(S_MIN), int(V_MIN)),
    ...
    best = None  # (score, cnt, rect, box_proc)
    ...
    for cnt in contours:
        ...
        # Rectangle-likeness score: 1.0 is perfect rectangle filled
        rect_score = float(area) / rect_area  # <= 1.0 typically
        if rect_score < float(RECT_SCORE_MIN):
            continue

        # Aspect ratio gate (on fitted rect)
        aspect = (rw / rh) if rw >= rh else (rh / rw)
        if aspect < float(MIN_ASPECT) or aspect > float(MAX_ASPECT):
            continue

        # Solidity gate (lets you allow ragged edges by lowering threshold)
        ...
        # Keep the best by rect_score * area (stable)
        score = rect_score * area
        box = cv2.boxPoints(rect).astype(np.float32)  # in proc coords
        if best is None or score > best[0]:
            best = (score, cnt, rect, box, rect_score)

    if best is None or homography is None:
        return {"mask": mask}

    _, cnt, rect, box_proc, rect_score = best
    ...
    # --- NEW: long-side orientation angle (0..90) ---
    # OpenCV angle is weird; this makes "longest side horizontal" => 0 deg,
    # and "longest side vertical" => 90 deg.
    long_angle = angle
    if rw < rh:
        long_angle = angle + 90.0

    long_angle = abs(long_angle)           # make it positive
    long_angle = max(0.0, min(90.0, long_angle))  # clamp to [0, 90]
    ...
    return {
        ...
        "angle": float(angle),                 # raw OpenCV angle (debug)
        "long_angle": float(long_angle),       # <-- use this for robot U
        "rect_wh": (float(rw), float(rh)),     # debug
...
def main():
    ...
    # Move robot out of the way BEFORE initial calibration
    ...
    try:
        while True:
            ...
            # Update detection parameters from sliders (HSV + morphology + scoring)
            read_tuning_sliders(WINDOW_NAME)

            # Detect + always get a mask back
            result = detect_rectangle(frame_bgr, homography)

            # Show mask window no matter what
            cv2.imshow("Mask", result.get("mask", np.zeros((10, 10), dtype=np.uint8)))

            # Draw on a copy
            disp = frame_bgr.copy()

            # Status line (safety default is paused)
            ...
            # Debug/tuning overlay (matches the new slider set)
            cv2.putText(
                disp,
            ...
            # Only treat as a valid detection if the detector returned a box
            if "box" in result:
                box = result["box"]
                ...
                # Compute robot command point
                x_cmd = rx + ROBOT_X_OFFSET
                y_cmd = ry + ROBOT_Y_OFFSET

                # Use the long-side angle for U
                u_cmd = result["long_angle"] + ROBOT_U_OFFSET

                # Save last command (for step-send)
                last_cmd = (x_cmd, y_cmd, u_cmd)

                # Rate-limit + minimum motion (only when continuous sending is ON)
                now = time.time()
                moved_enough = True
